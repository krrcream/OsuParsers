...
namespace OsuParsers.Enums
{
    ...
    public class BoolMatrix
    {
        ...
        public bool[,] GetData()
        {
            var result = new bool[_rows, _cols];
            var resultFlat = MemoryMarshal.CreateSpan(ref result[0, 0], _data.Length);
            _data.AsSpan().CopyTo(resultFlat);
            return result;
        }
        ...
        /// 复制数据到另一个矩阵
        /// </summary>
        public void CopyTo(BoolMatrix target)
        {
            if (Rows != target.Rows || Cols != target.Cols)
                throw new ArgumentException("Matrix dimensions must match");
            Array.Copy(_data, target._data, _data.Length);
        ...
        /// 从另一个矩阵复制数据
        /// </summary>
        public void CopyFrom(BoolMatrix source)
        {
            source.CopyTo(this);
        }
        ...
        /// 获取行数据作为 Span（用于 MemoryMarshal）
        /// </summary>
        public Span<bool> GetRowSpan(int row)
        {
            ThrowIfInvalidRow(row);
            return MemoryMarshal.CreateSpan(ref _data[row * _cols], _cols);
        }
        ...
        {
            var clone = new BoolMatrix(Rows, Cols);
            CopyTo(clone);
            return clone;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Span<bool> AsSpan() => _data.AsSpan();
