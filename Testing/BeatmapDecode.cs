...
public class BeatmapDecode
{
    public static void Run()
    {
        ...
        string fullPath1 = Path.Combine(projectRoot, "DecodeTest",
            "Tino & Cha Shao Jun feat. Orihara RuruMashiro KanonHiiroLing Yuan Yousa - Chun Ri You (krrcream) [(LV.13) Another].osu");
        string fullpath2 = Path.Combine(projectRoot, "DecodeTest",
            "32ki feat. Hatsune Miku & Kasane Teto - Mesmerizer (krrcream) [(LV.14) Another].osu");
        Beatmap beatmap1 = BeatmapDecoder
            .Decode(fullPath1);
        //HOW TO USE (WHEN MODE IS MANIA)
        //用.AsManiaNote()方法将普通note转为maniaNote对象来使用maniaNote的特定方法
        beatmap1.HitObjects.UpdateHitObject(1,
            beatmap1.HitObjects[1].AsManiaNote()
                .CloneNote(EndTime: 5555)); //如果是note，EndTime无法直接修改，必须要克隆面条对象。这是安全替换坐标i的对象方法
        ...
        Console.WriteLine(
            $"Beford change HoldLength: {beatmap1.HitObjects[2].EndTime}-{beatmap1.HitObjects[2].StartTime}={beatmap1.HitObjects[2].EndTime - beatmap1.HitObjects[2].StartTime}");
        LN.HoldLength = 9999;
        Console.WriteLine($"Run LN.HoldLength = 9999;");
        Console.WriteLine(
            $"After change HoldLength: {beatmap1.HitObjects[2].EndTime}-{beatmap1.HitObjects[2].StartTime}={beatmap1.HitObjects[2].EndTime - beatmap1.HitObjects[2].StartTime}");
        Console.WriteLine();
